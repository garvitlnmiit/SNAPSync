...
static int readfd(int fd, char *buffer, size_t N);
static int writefd(int fd, const char *buf, size_t len);
static int writefd_unbuffered(int fd, const char *buf, size_t len);
static int mplex_write(int fd, enum msgcode code, const char *buf, size_t len, int convert);
...
static int mplex_write(int fd, enum msgcode code, const char *buf, size_t len, int convert)
{
	...
	if((writefd_unb_retval=writefd_unbuffered(fd, buffer, n+4))==-19)/*garvit*/
		return -19;
	...
	if (len > n){
		...
		if((writefd_unb_retval=writefd_unbuffered(fd, buf+n, len-n))==-19)
		return -19;
	}
	if (!--defer_forwarding_messages && !no_flush)
		msg_flush();
...
static int whine_about_eof(int fd)
{
...
static int read_timeout(int fd, char *buf, size_t len)
{
	...
	while (cnt == 0) {
		...
		if (n <= 0) {
			if (n == 0)
				retn=whine_about_eof(fd); /* Doesn't return. */
				if(retn==-17)
				return -17;
...
static int readfd_unbuffered(int fd, char *buf, size_t len)
{
	...
	if (!io_multiplexing_in && iobuf_in_remaining == 0) {
		...
		if(iobuf_in_remaining==(size_t)-17)
		return -17;
		if(iobuf_in_remaining==(size_t)-19)
			return -19;
	...
	while (total < len) {
		...
		if (count <= 0) {
			if (count < 0 && errno == EBADF)
				exit_cleanup(RERR_SOCKETIO);
			//check_timeout();
			check_time_retval=check_timeout();
			if(check_time_retval==-19)
			return -19;
			continue;
		}
...
int io_flush(int flush_it_all)
{
	...
	if (iobuf_out_cnt) {
		if (io_multiplexing_out){
			...
			if((mplex_write_retval=mplex_write(sock_f_out, MSG_DATA, iobuf_out, iobuf_out_cnt, 0))==-19)
			return -19;
		}else{
			//mplex_write(sock_f_out, MSG_DATA, iobuf_out, iobuf_out_cnt, 0);
			writefd_unb_retval=writefd_unbuffered(iobuf_f_out, iobuf_out, iobuf_out_cnt);
			if(writefd_unb_retval==-19)
			return -19;
		}
...
static int writefd(int fd, const char *buf, size_t len)
{
	...
	if (fd == write_batch_monitor_out){
		if((writefd_unb_retval=writefd_unbuffered(batch_fd, buf, len))==-19)
		return -19;
	}
	if (!iobuf_out || fd != iobuf_f_out) {
		if((writefd_unb_retval=writefd_unbuffered(fd, buf, len))==-19)
		return -19;
		return 5;
	}
	...
	while (len) {
		...
		if (iobuf_out_cnt == IO_BUFFER_SIZE){
			if((io_flush_retval=io_flush(NORMAL_FLUSH))==-19)
				return -19;
		}
	}
return 5;
...
int write_buf(int f, const char *buf, size_t len)
{
	return writefd(f,buf,len);
}
