...
static int matched(int f, struct sum_struct *s, struct map_struct *buf,
...
//send_token(f, i, buf, last_match, n, i < 0 ? 0 : s->sums[i].len);
	if((st_ret=send_token(f, i, buf, last_match, n, i < 0 ? 0 : s->sums[i].len))==-19)
		return -19;
 ...
 **/
int match_sums(int f, struct sum_struct *s, struct map_struct *buf, OFF_T len)
{
	...
	if (len > 0 && s->count > 0) {
		...
		{	//matched(f, s, buf, j, -2);
			if((m_ret=matched(f, s, buf, j, -2))==-19)
				return -19;
		}	
		if((m_ret=matched(f, s, buf, len, -1))==-19)
			return -19;
		//matched(f, s, buf, j, -2);
	}
