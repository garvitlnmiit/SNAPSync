 ...
 **/
void _exit_cleanup_my(int code, const char *file, int line)
{
...
#include "case_N.h"
		...
		if (cleanup_child_pid_my != -1) {
			int status;
			int pid = wait_process(cleanup_child_pid_my, &status, WNOHANG);
			if (pid == cleanup_child_pid_my) {
...
#include "case_N.h"
		...
		if (cleanup_got_literal_my && cleanup_fname && cleanup_new_fname
