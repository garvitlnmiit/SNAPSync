...
static int simple_send_token(int f, int32 token, struct map_struct *buf,
			      OFF_T offset, int32 n)
{
	...
	if (n > 0) {
		...
		while (len < n) {
			...
			if((wb_ret=write_buf(f, map_ptr(buf, offset+len, n1), n1))==-19)
				return -19;
			len += n1;
		}
	}
 ...
 */
int send_token(int f, int32 token, struct map_struct *buf, OFF_T offset,
		int32 n, int32 toklen)
{
	int sst_ret;
	if (!do_compression){
		//simple_send_token(f, token, buf, offset, n);
		if((sst_ret=simple_send_token(f, token, buf, offset, n))==-19)
			return -19;
	}else
